...
const ResultsSwiperControls = forwardRef<SwiperType | null>((_, swiperRef) => {
  const BUTTON_STYLE =
  ...
  const handleSwiper = (direction: "left" | "right") => {
    if (swiperRef && typeof swiperRef === "object" && "current" in swiperRef) {
      if (direction === "left") {
...
export default ResultsSwiperControls
