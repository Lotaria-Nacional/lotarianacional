...
export default function SkilledStaffJobDetailsPage() {
  ...
  const { title, description, department: dept, responsabilities, requirements } = job.data;
      ...
      <main className="relative grid w-full grid-cols-1 lg:grid-cols-[2fr_1fr] gap-12">
        <section className="w-full flex flex-col gap-4">
          ...
          <header className="flex flex-col gap-3">
            {dept && <span className="px-2 capitalize text-[12px] bg-[#f5f5f5] w-fit rounded-full">{dept}</span>}
          ...
          {/* Descrição */}
          {description && typeof description === 'string' && <p className="text-justify text-sm">{description}</p>}
